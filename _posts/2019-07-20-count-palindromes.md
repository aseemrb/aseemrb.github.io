...
Here, `ALPH` is the size of the alphabet $$\Sigma$$, required to enable adding edges for each possible letter. The value of `Tree[u].envelope[i]` stores the index `v` such that `Tree[v]` represents the palindrome `i Tree[u] i`; `renvelope` is just the reverse map for `envelope`. Note that `i` represents a single letter here and `Tree[u]` represents the palindrome at index `u`. Thus, `envelope` maintains the **border edges** defined above.
