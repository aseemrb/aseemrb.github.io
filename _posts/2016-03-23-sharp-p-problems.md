...
To prove the correspondence, take two cases for a cycle cover C of graph $$G$$:

- If edge u-v was not in C, then to cover all the edges in the transformed graph, we must use all the self-loops, so the total contribution  = 1 in product, hence no change in the output. This is as good as not considering the edge u-v in the original graph
